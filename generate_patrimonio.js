...
import { Plus, Edit2, Trash2, Save, Search, Package, Layers, MapPin, BarChart3, AlertCircle, Camera, QrCode } from 'lucide-react';
...
export default function PatrimonioPage() {
  ...
  const canEditPatrimonio = user?.id_master || user?.is_admin || hasPermission('patrimonio:editar') || hasPermission('patrimonio:novo');
  const canReadCategorias = user?.id_master || user?.is_admin || hasPermission('/patrimonio_categorias') || hasPermission('patrimonio_categorias');
  const canEditCategorias = user?.id_master || user?.is_admin || hasPermission('patrimonio_categorias:editar') || hasPermission('patrimonio_categorias:novo');
  ...
  const handleDeleteCategoria = async (id: string) => {
    confirmDelete(async () => {
      ...
      fetchCategorias();
    }, 'Tem certeza que deseja excluir esta categoria? Bens associados podem perder a referência.');
  };
  ...
  const handleDeleteBem = async (id: string) => {
    confirmDelete(async () => {
      ...
      fetchBens();
    }, 'Tem certeza que deseja dar baixa/excluir este bem?');
  };
              ...
              <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div className="relative flex-1 max-w-md">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-slate-800 border-none rounded-2xl text-sm font-medium focus:ring-2 focus:ring-amber-500 outline-none"
                  />
                </div>
                {canEditPatrimonio && (
                  <button
