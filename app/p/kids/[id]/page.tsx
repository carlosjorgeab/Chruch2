...
export default function PublicChildOverview() {
  ...
  const [comunicadosGerais, setComunicadosGerais] = useState<Comunicado[]>([]);
  const [comunicadosEspecificos, setComunicadosEspecificos] = useState<Comunicado[]>([]);
    ...
    async function loadPublicChildData() {
      try {
        ...
        if (salaData) {
          ...
          if (comsData) {
            const gerais: Comunicado[] = [];
            const especificos: Comunicado[] = [];

            comsData.forEach((com: any) => {
              const targetIds = parseCriancasIds(com.criancas_ids);
              if (targetIds.includes('all')) {
                gerais.push(com);
              } else if (targetIds.includes(childData.id)) {
                especificos.push(com);
              }
            });

            setComunicadosGerais(gerais);
            setComunicadosEspecificos(especificos);
          }
        }
    ...
    <>
      ...
      <div className="child-page-wrapper min-h-screen py-8 px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto space-y-8">
          ...
          {/* Communications Block: General and Specific */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            
            {/* PANEL 1: Comunicados Gerais */}
            <div className="child-panel border rounded-[2.5rem] p-8 shadow-sm space-y-6">
              <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800/60 pb-3">
                <div className="flex items-center gap-2">
                  <Megaphone size={18} className="text-indigo-500" />
                  <h3 className="text-xs font-black uppercase tracking-widest">📢 Comunicados Gerais (Sala)</h3>
                </div>
              ...
              {comunicadosGerais.length === 0 ? (
                <div className="text-center py-12 bg-slate-50/50 dark:bg-slate-955/10 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 space-y-2">
                  <Megaphone size={28} className="text-slate-300 dark:text-slate-600 mx-auto" />
                  <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Sem avisos gerais</p>
                  <p className="text-[10px] text-slate-400">Nenhum aviso geral enviado para esta sala hoje.</p>
                </div>
              ) : (
                    ...
                    return (
                      <div key={comunicado.id} className="p-4 bg-slate-50/50 dark:bg-slate-950/20 rounded-2xl border border-slate-150/40 dark:border-slate-850/60 space-y-3">
                        <div className="flex items-center justify-between gap-2">
                          <span className="px-2.5 py-0.5 text-[9px] font-black uppercase tracking-wider rounded-full border bg-indigo-500/10 text-indigo-600 border-indigo-500/20">
                            ...
                            <div className="flex flex-col gap-1">
                              {files.map((file, fIdx) => (
                                <a 
                                  ...
                                  target="_blank"
                                  rel="noreferrer"
                                  className="inline-flex items-center gap-1.5 text-xs text-indigo-600 dark:text-indigo-400 hover:underline bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-2 rounded-xl"
                                >
                                  <FileText size={12} />
            ...
            {/* PANEL 2: Comunicados Específicos */}
            <div className="child-panel border rounded-[2.5rem] p-8 shadow-sm space-y-6">
              <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800/60 pb-3">
                <div className="flex items-center gap-2">
                  <Star size={18} className="text-indigo-500" />
                  <h3 className="text-xs font-black uppercase tracking-widest">📝 Comunicados Individuais</h3>
                </div>
                <span className="text-[9px] font-black uppercase tracking-widest bg-amber-500/10 text-amber-750 dark:text-amber-400 px-2.5 py-1 rounded-md">
                  {comunicadosEspecificos.length}
                </span>
              </div>

              {comunicadosEspecificos.length === 0 ? (
                <div className="text-center py-12 bg-slate-50/50 dark:bg-slate-955/10 rounded-2xl border border-dashed border-slate-200 dark:border-slate-800 space-y-2">
                  <Star size={28} className="text-slate-300 dark:text-slate-600 mx-auto" />
                  <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Nenhum aviso individual</p>
                  <p className="text-[10px] text-slate-400">Nenhum comunicado individual para {displayNome.split(' ')[0]} hoje.</p>
                </div>
              ) : (
                <div className="space-y-4 max-h-[400px] overflow-y-auto pr-1">
                  {comunicadosEspecificos.map((comunicado) => {
                    const dateObj = new Date(comunicado.created_at);
                    const formattedDate = dateObj.toLocaleDateString('pt-BR', {
                      day: '2-digit',
                      month: '2-digit',
                    ...
                    return (
                      <div key={comunicado.id} className="p-4 bg-slate-50/50 dark:bg-slate-950/20 rounded-2xl border border-slate-150/40 dark:border-slate-850/60 space-y-3">
                        <div className="flex items-center justify-between gap-2">
                          <span className="px-2.5 py-0.5 text-[9px] font-black uppercase tracking-wider rounded-full border bg-amber-500/10 text-amber-700 border-amber-500/20">
                            {comunicado.tipo || 'Individual'}
                          </span>
                          <span className="text-[9px] text-slate-400 font-bold">{formattedDate}</span>
                        </div>
                        <p className="text-xs text-slate-800 dark:text-slate-200 leading-relaxed font-semibold whitespace-pre-line">
